...
class SQLiteWrapperTest extends PHPUnit_Framework_TestCase {

	protected $dbConn;

	public function __construct(){
		if(null == $this->dbConn){
			$pdo = new Chevron\PDO\SQLite\Wrapper( TEST_DB_SQLITE_DSN, "", "" );
			$this->dbConn = $pdo;
		}
		return $this->dbConn;
	}
	...
	public function testKeyRows(){
		...
		// standard
		$result = $this->dbConn->keyrows($sql, array(2,3));
		$this->assertInternalType("array", $result, "Wrapper::keyrow did not return array");
		$this->assertEquals($result, $expected, "Wrapper::keyrow returned the wrong result");

	}

	public function testKeyRowsIn(){
		// $this->markTestSkipped("Wrapper requires an active DB connection");
		...
		// using IN
		$result = $this->dbConn->keyrows($sql, array(array(2,3)), true);
		$this->assertInternalType("array", $result, "Wrapper::keyrow (in) did not return array");
		$this->assertEquals($result, $expected, "Wrapper::keyrow (in) returned the wrong result");

	}

	public function testKeyRowsEmptyResult(){
		// $this->markTestSkipped("Wrapper requires an active DB connection");

		$sql = "select widget_id, type_id from widgets where widget_id in (%s) order by widget_id;";

		// empty
		$result = $this->dbConn->keyrows($sql, array(array(12,13)), true);
		$this->assertInternalType("array", $result, "Wrapper::keyrow (empty result) did not return array");
		$this->assertEquals($result, array(), "Wrapper::keyrow (empty result) returned the wrong result");
	}
	...
	public function testInsertFuncs(){
		...
		$data = array(
			...
			"descr"     => array(true, "hex(randomblob(16))"),
			"type_id"   => "26",
			"updated"   => array(true, "date('now')"),
		);
	...
	public function testReplaceFull(){
		...
		// replace full ... we expect 1 because of a INSERT OR REPLACE
		$result = $this->dbConn->replace("sprockets", $data);
		$this->assertEquals($result, 1, "Wrapper::replace (full) didn't insert");
	...
	public function testReplacePartial(){
		...
		// replace partial ... we expect 1 because of a INSERT OR REPLACE
		$result = $this->dbConn->replace("sprockets", $data);
		$this->assertEquals($result, 1, "Wrapper::replace (partial) didn't insert");
	...
	public function testMultiReplace(){
		...
		// replace full ... we expect 2 because of a INSERT OR REPLACE
		$result = $this->dbConn->multi_replace("sprockets", $data);
		$this->assertEquals($result, 2, "Wrapper::replace (multi) didn't insert");
	...
	public function testUpdateOnDuplicateKey(){
		...
		// updates report 1 affected row. if the update is 0, insert.
		// there will always only ever be 1 row affected
	...
	public function testInsertOnDuplicateKey(){
		...
		$result = $this->dbConn->row($sql);
		$this->assertEquals($expected, $result, "Wrapper::update did not verify");
		$this->assertInternalType("null", $result["updated"], "Wrapper::update did not verify");
	}
