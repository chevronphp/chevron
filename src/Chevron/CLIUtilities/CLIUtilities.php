...
abstract class CLIUtilities {

	static function format( $msg, $attrs = array() ){
		...
		foreach ( (array)$attrs as $attr ){
			switch( true ){
				case strpos( $attr, "fg:" ) !== false :
					$color = substr( $attr, 3 );
					$code[] = sprintf( "3%s", $colors[ $color ] );
					break;
				case strpos( $attr, "bg:" ) !== false :
					$color = substr( $attr, 3 );
					$code[] = sprintf( "4%s", $colors[ $color ] );
					break;
				case strpos( $attr, ":" ) === false :
	...
	static function indent( $str, $spaces, $length = 76 ){
	...
	static function confirm( $msg = "" ){
		...
		printf( "\n\n%s (y/n) ", format( $msg, "reverse" ) );
		return trim( fgets( STDIN ) ) == "y";
	}

	static function choose( $msg = "" ){
		$msg = $msg ?: "Are you sure?";
		printf( "\n\n%s ", format( $msg, "reverse" ) );
		return trim( fgets( STDIN ) );
	}

	static function clean_exit( $msg = "", $spaces = 0 ){
	...
	static function man( $array ){
		...
		foreach( $array as $type => $lines ){
			switch( $type ){
				case( "title" ):
					$final .= format( rtrim( $lines ), "underscore" );
					$final .= "\n\n";
				break;
				case( "description" ):
					foreach( (array)$lines as $line ){ ++$n;
						$final .= indent( rtrim( $line ), 0 ) . "\n\n";
					}
				break;
				case( "args" ):
					$final .= format( "ARGS", "underscore" );
					$final .= "\n\n";
					foreach( (array)$lines as $arg => $arg_descr ){
						$final .= sprintf( "   %s\n%s\n\n", rtrim( $arg ), indent( rtrim($arg_descr), 8 ) );
					}
					$final .= "\n\n";
				break;
			}
